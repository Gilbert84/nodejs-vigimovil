        ...
        .exec(
            (err, viajes) => {
                ...
                Viaje.populate(viajes, [
                        { path: 'asignacion.operario', select: 'nombre identificacion img', model: 'Operario' },
...
// ==========================================
// Obtener viaje por operario
// ==========================================
app.get('/cargarViajePorOperario/:id', (req, res) => {
    ...
    Asignacion.findOne({ operario:_id}, (err, asignacionEncontada) => {
        ...
        if (!asignacionEncontada) {
            return res.status(200).json({
                ok: false,
                mensaje: 'No se obtubo ningun resultado',
                errors: err
            });
        }

        Viaje.findOne({ asignacion:asignacionEncontada._id , estado:{ codigo: 0 }}, (err, viajeEncontrado) => {
            ...
            if (!viajeEncontrado) {
                return res.status(200).json({
                    ok: false,
                    mensaje: 'No se obtubo ningun resultado al cargar el viaje',
                    errors: err
                });
            }
...
app.put('/:id', mdAutenticacion.verificaToken, (req, res) => {
    ...
    Viaje.findById(id, (err, viaje) => {
        ...
        viaje.numeroDespacho = body.numeroDespacho;
