...
app.get('/coleccion/:tabla/:busqueda', (req, res) => {
    ...
    switch (tabla) {
        ...
        case 'viajes':
            //promesa = buscarViajes(busqueda, regex);
...
function buscarViajes(busqueda, regex) {

    return new Promise((resolve, reject) => {

        Viaje.find({}, 'numeroDespacho')
            .or([{ 'numeroDespacho': regex }])
            .populate('asignacion')
            .populate('ruta')
            .exec((err, viajes) => {

                if (err) {
                    reject('Error al cargar viajes', err);
                } 
                else {
