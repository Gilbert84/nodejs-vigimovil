...
io.on('connection', (socket) => {
    ...
    socket.on('actualizarAsignaciones', (data, callback) => {
        ...
        asignaciones.obtener().then((asignacionesActuales) => {
            socket.broadcast.emit('asigancionesActuales', asignacionesActuales);
            return callback(asignacionesActuales);
        });
    });
    ...
    socket.on('obtenerViajeOperario', (operario, callback) => {
        if (!operario._id) {
            return callback({ error: true, server: 'el id es requerido' });
        }
        let viajeOperario = asignaciones.obtenerViajeOperario(operario._id);
        return callback(viajeOperario);
    });
    ...
    socket.on('dispositivoConectado', (dispositivo, callback) => {
        ...
        socket.broadcast.emit('listaActualDispositivos', dispositivos.agregarDispositivo(dispositivo));
        database.actualizar(dispositivo).then((resp) =>{
            console.log('log:73 dispositivo conectado :',resp);
    ...
    socket.on('dispositivoMensajeTodos', data => {
        let dispositivo = dispositivos.obtenerDispositivo(socket.id);
        let mensaje = {
    ...
    socket.on('confirmacionAsignacion', (data, callback) => {
        ...
        socket.broadcast.emit('confirmacionAsignacion', confirmacionAsignacion);
        return callback({
            ok: true,
    ...
    socket.on('registroDispositivo', (dispositivo, callback) => {
        ...
        dispositivos.crearDispositivo(dispositivo)
            .then((resp) => {
                return callback(resp); //devuelvo el callback al dispositivo
            })
            .catch((err) => {
                return callback(err); //devuelvo el callback al dispositivo
            });
    ...
    socket.on('atenderDespacho', (data, callback) => {
        ...
        return callback(atenderDespacho);
    ...
    socket.on('disconnect', function() {
        ...
        if (dispositivoBorrado !== undefined) {
            dispositivoBorrado.disponible = false;
            socket.broadcast.emit('listaActualDispositivos', dispositivos.obtenerDispositivos());
            database.actualizar(dispositivoBorrado).then((resp) => {
                console.log('log:269 dispositivo desconectado:',resp);
            });
        }
